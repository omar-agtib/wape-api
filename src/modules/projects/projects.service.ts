...
export class ProjectsService {
  ...
  async findAll(
    ...
    if (filters.startDateFrom)
      qb.andWhere('p.start_date >= :from', { from: filters.startDateFrom });
    if (filters.startDateTo)
      qb.andWhere('p.start_date <= :to', { to: filters.startDateTo });
  ...
  async findOne(
    tenantId: string,
    id: string,
  ): Promise<Project & { financeSnapshot?: ProjectFinanceSnapshot }> {
    ...
    const financeSnapshot = await this.snapshotRepo.findOne({
      where: { projectId: id },
    });
    ...
    if (dto.budget !== undefined) {
      await this.snapshotRepo.update(
        { projectId: id },
        {
          ...
          remainingBudget:
            dto.budget -
            (await this.snapshotRepo.findOne({ where: { projectId: id } }))!
              .totalSpent,
        },
      );
    }
  ...
  async getFinance(tenantId: string, projectId: string) {
    ...
    if (!snapshot) {
      throw new NotFoundException({
        ...
        message: 'Finance snapshot not found',
      });
    }
  ...
  async recalculateFromTasks(
    ...
    const avgProgress =
      taskStatuses.length > 0
        ? taskStatuses.reduce((sum, t) => sum + t.progress, 0) /
          taskStatuses.length
        : 0;
    ...
    const allCompleted =
      taskStatuses.length > 0 &&
      taskStatuses.every((t) => t.status === 'completed');

    let newStatus = project.status;
    if (allCompleted) newStatus = ProjectStatus.COMPLETED;
    else if (hasOnProgress && project.status === ProjectStatus.PLANNED)
      newStatus = ProjectStatus.ON_PROGRESS;
