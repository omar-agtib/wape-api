...
import {
  IsArray, IsIn, IsNotEmpty, IsNumber,
...
export class CreatePoLineDto {
  ...
  @ApiProperty({ example: 82.50, description: 'Negotiated unit price' })
...
export class CreatePurchaseOrderDto {
  @ApiProperty({ example: 'uuid-of-supplier-contact', description: 'Must be a contact with contactType=supplier (RG08)' })
  @IsUUID()
  supplierId: string;

  @ApiPropertyOptional({ example: 'uuid-of-project', description: 'Optional project link (C8)' })
  @IsOptional() @IsUUID()
  projectId?: string;
  ...
  @ApiProperty({ type: [CreatePoLineDto], description: 'At least one line required' })
  @IsArray()
