...
export class ContactsService {
  ...
  async findAll(tenantId: string, filters: ContactFilterDto): Promise<PaginatedResult<Contact>> {
    ...
    if (filters.contactType) qb.andWhere('c.contact_type = :type', { type: filters.contactType });
    if (filters.search) {
      qb.andWhere('(c.legal_name ILIKE :s OR c.email ILIKE :s)', { s: `%${filters.search}%` });
    }

    qb.orderBy('c.legal_name', 'ASC').skip((page - 1) * limit).take(limit);
    const [items, total] = await qb.getManyAndCount();
    return paginate(items, total, page, limit);
  }

  async findOne(tenantId: string, id: string): Promise<Contact & { documents: ContactDocument[] }> {
    const contact = await this.contactRepo.findOne({ where: { id, tenantId } });
    if (!contact) {
      throw new NotFoundException({ error: 'CONTACT_NOT_FOUND', message: `Contact '${id}' not found` });
    }
  ...
  async verifyType(tenantId: string, id: string, expectedType: ContactType): Promise<Contact> {
    const contact = await this.findOneRaw(tenantId, id);
    if (contact.contactType !== expectedType) {
      ...
      throw new UnprocessableEntityException({
        ...
        details: { contactId: id, actualType: contact.contactType, expectedType },
      });
    }
    return contact;
  }
